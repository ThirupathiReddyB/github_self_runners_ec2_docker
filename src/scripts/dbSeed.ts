...
import { getCoordinatesFromAddress } from "../utility/helperFunction/contentManagement.services.helper";

async function main() {

    const findAllFac = await prisma.facility.findMany({
        where: {
            lat: 0,
            long: 0
        }
    ...
    for (const fac of findAllFac) {
        try {
            const { lat, lng } = await getCoordinatesFromAddress(fac.facAddress, fac.facPincode)
